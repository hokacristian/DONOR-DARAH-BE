...
const { calculateForExamination } = require('../../services/mooraService');
...
exports.createDonorWithExamination = async (req, res, next) => {
  try {
    ...
    // Trigger MOORA calculation
    const mooraResult = await calculateForExamination(result.examination.id);
...
exports.updateDonorWithExamination = async (req, res, next) => {
  try {
    // TODO: Implement with MOORA recalculation
    res.status(501).json({ message: 'Not implemented yet' });
  } catch (error) {
    next(error);
  }
